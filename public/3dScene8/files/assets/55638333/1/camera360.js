...
var Camera360 = pc.createScript('camera360');

Camera360.attributes.add("mouseLookSensitivity", {type: "number", default: 0, title: "Mouse Look Sensitivity", description: ""});
Camera360.attributes.add("touchLookSensitivity", {type: "number", default: 0, title: "Touch Look Sensitivity", description: ""});

Camera360.attributes.add("sphere", {type: "entity", title: "Sphere"});

Camera360.prototype.initialize = function () {
    var self=this;
...
Camera360.prototype.deact = function () {
    var self=this;
     setTimeout(function(){
         var quat = self.entity.getLocalRotation();
         self.ex = self.getPitch(quat) * pc.math.RAD_TO_DEG;
...
Camera360.prototype.teleport = function (targetFloorPosition) {
...
Camera360.prototype.addEventCallbacks = function() {
    if (this.app.mouse) {
...
Camera360.prototype.removeEventCallbacks = function() {
    if (this.app.mouse) {
...
Camera360.prototype.getYaw = function (quaternion) {
    var q = quaternion;
    var x = q.x * q.x;
    var y = q.y * q.y;
...
Camera360.prototype.getPitch = function(quaternion) {
    var q = quaternion;
    return -Math.asin(2 * q.z * q.y + 2 * q.x * q.w);
};

Camera360.prototype.update = function (dt) {
...
Camera360.prototype.moveCamera = function(dx, dy, lookSensitivity) {
     if(!this.canLook){ return; }
...
Camera360.prototype.onMouseMove = function (event) {    
    if (!this.lmbDown)
        return;
...
Camera360.prototype.onMouseDown = function (event) {
    if (event.button === 0) {
        this.lmbDown = true;
    }
};

Camera360.prototype.onMouseUp = function (event) {
    if (event.button === 0) {
        this.lmbDown = false;
    }
};

Camera360.prototype.onTouchStart = function(event) {
...
Camera360.prototype.onTouchEnd = function(event) {
...
Camera360.prototype.onTouchMove = function(event) {
    var touch = event.touches[0];
    if (event.touches.length == 1) {
...
Camera360.prototype.onTouchCancel = function(event) {
    this.lmbDown = false;
};
