...
PathFollower.attributes.add("pathRoot", {type: "entity", title: "Path Root"});
PathFollower.attributes.add("duration", {type: "number", default: 10, title: "Duration Secs"});

PathFollower.attributes.add("startTime", {
    type: "number", 
    default: 0, 
...
PathFollower.prototype.setRandomStart = function() {
    this.startTime= Math.floor(Math.random() * this.duration);
       this.time = pc.math.clamp(this.startTime, 0, this.duration);    
};

/*
PathFollower.prototype.setRandomStart = function(dur) {
    this.duration=dur;
    this.startTime= Math.floor(Math.random() * this.duration);
};
*/
...
PathFollower.prototype.update = function(dt) {
    ...
    var tXx = this.px.value(percent+0.01);
    var tZz = this.pz.value(percent+0.01);
    this.entity.lookAt(new pc.Vec3(tXx, -1, tZz));
...
PathFollower.prototype.createPath = function () {
...
// PathFollower.prototype.swap = function(old) { };
